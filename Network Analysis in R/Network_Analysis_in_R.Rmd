...
The following documentoutlines the written portion of the lessons from [DataCamp](https://www.datacamp.com/)'s [Network Analysis in R course](https://www.datacamp.com/courses/network-analysis-in-r). This primarily uses the "igraph" package and requires Intermediate knowledge of R.
...
This document was created by Neil Yetz on 02/14/2018. Please send any questions or concerns in this document to Neil at <ndyetz@gmail.com>
...
chimps          <- read_csv("monkeys.csv")
kc              <- as.matrix(read_csv("karate_class.csv", col_names = FALSE))
...
#NOTE FROM NEIL: Creating object, of "genders" & "ages" This was not included in the Datacamp course
...
```{r, echo = FALSE}
#NOTE FROM NEIL: Creating object, of "hours" This was not included in the Datacamp course
...
What term is typically used to describe the relative strength of edges? (Answer is **Bolded**)
...
```{r}
#NOTE FROM NEIL: To solve this problem, use the following code:

# Find all edges that include "Jasmine"
#E(g1)[[inc('Jasmine')]]  # <- This Code!
...
What is the longest possible path in a network referred to as? (Answer is **Bolded**)
...
Which measure of a vertex importance is determined by how often that vertex lies on the shortest path between any two vertices in a network? (Answer is **Bolded**)
...
Using the function `edge_density()` calculate the density of the graph `g` and assign this value to the vector `gd`.
...
If a graph has 7 vertices there are 21 possible edges in the network. If 14 edges exist, what is the density of the network? (Answer is **Bolded**)
...
# Create two new undirected induced_subgraphs, each containing only the vertices of each largest clique.
...
```{r, echo = FALSE}
g <- graph_from_edgelist(as.matrix(chimps), directed = TRUE)
```
...
g <- graph.edgelist(kc, directed = FALSE)
...
##Note from Neil Regarding the below Interactive network sections
Unfortunately as of 03/07/18 the `threejs` package does not properly render to rStudio or R-notebooks in general. The edge lines will not appear properly. To properly view the threejs plots (using function graphjs()) you need to plot it in your console, to view it in the viewer window, and show it in a new window. This will open it up in an html document and the edges will properly show. If you find this package ever begins to work please email me to let me know! at <ndyetz@gmail.com>!

##Interactive networks with threejs

In this course you have exclusively used `igraph` to make basic static network plots. There are many packages available to make network plots. One very useful one is `threejs` which allows you to make interactive network visualizations. This package also integrates seamlessly with `igraph`. In this exercise you will make a basic interactive network plot of the karate club network using the `threejs` package. 

To properly view the edges in the 3D graphs, you must first knit the document to an html and proceed to open it up in a web browser (Works in Google Chrome). Please note, the HTML document will **NOT** show edges to the 3d graphs when it is open in R-studio. it must be open in an web browser. 
...
##Edges not showing up on my computer -Neil
