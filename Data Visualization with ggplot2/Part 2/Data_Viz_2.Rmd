...
barley    <- read_csv("barley.csv")
adult     <- readRDS("chis2009.rds")


#get random subset of adult dataset
adult <- sample_n(adult, 44346)
```
...
```{r, echo = FALSE}
#Create mtcars.cyl dataset

names <-  c("cyl", "wt.avg", "sd", "n", "prop")

mtcars.cyl <- as_data_frame(
                            matrix(
                                    c( 
...
`GGally::ggpairs(mtcars2)` will produce the plot of a selection of the mtcars dataset, mtcars2, shown below. Depending on the nature of the dataset a specific plot type will be produced and if both variables are continuous the correlation (rho) will also be calculated.
...
```{r, echo = FALSE}
mtcars2 <- mtcars %>% 
  dplyr::select(mpg, disp, drat, wt, qsec)
  
GGally::ggpairs(mtcars2, progress = FALSE)
...
Add facetting with the same formula as in the heat map plot, instead of ncol, set nrow to 
...
ggplot(barley, aes(x = factor(year), y = yield, color = site, fill = site, group= site)) +
...
(max(adult$SRAGE_P) - min(adult$SRAGE_P))/30
```
...
adult$RACEHPR2 <- factor(adult$RACEHPR2, labels = c("Latino", "Asian", "African American", "White", "Other", "Other", "Other"))
