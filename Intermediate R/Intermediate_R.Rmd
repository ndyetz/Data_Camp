...
The following documentoutlines the written portion of the lessons from [DataCamp](https://www.datacamp.com/)'s ["Intermediate R" course](https://www.datacamp.com/courses/intermediate-r). This is a follow-up to the "Introduction to R course" That requires you to understand the basics of Base R.

As a note: All text is completely copied and pasted from the course. There are insances where the document refers to the "editor on the right", please note, that in this notebook document all of the instances are noted in the "r-chunks" (areas containing working r-code), which occurs below the text, rather than to the right. Furthermore, This lesson contained instructional videos at the beginnign of new concepts that are not detailed in this document. However, even without these videos, the instructions are quite clear in indicating what the code is accomplishing. 
...
This document was created by Neil Yetz on 09/24/2017. Please send any questions or concerns in this document to Neil at <ndyetz@gmail.com>
...
#Neil creating ttt Matrix (Only available in DataCamp)
...
In your workspace, there's a matrix ```ttt```, that represents the status of a [tic-tac-toe](https://en.wikipedia.org/wiki/Tic-tac-toe) game. It contains the values "X", "O" and "NA". Print out ```ttt``` in the console so you can have a closer look. On row 1 and column 1, there's "O", while on row 3 and column 2 there's "NA".
...
##Next, you break it (Next & Break statements)
...
#Note from Neil: Please note that the output this will not appear in the notebook, look in your"Help" tab in the lower right box!
...
**Possible Answers** (correct answer **BOLDED**)
...
#Neil Bonus! Let's test our function! 
pow_two(5)
pow_two(5, print_info = FALSE) #Notice how this code does NOT print, "5 to the power two equals 25" YPU CODED THAT!
```
...
```{r, echo = FALSE}
# Ignore this code
# Neil Defining the two_dice() function
...
*note from Neil: The function noted below has been defined in an above code chunk*
...
Can you tell which one of the following statements is __false__ about the following piece of code?
...
##R you functional? 
```{r, echo = FALSE}
#ignore this code correct data only available in DataCamp, need to redefine facebook and linked in vectors

#Neil redefining facebook and linkedin datasets
...
##R you functional? (2) [for loops in functions]
...
#install.packages("ggplot2") #Note from Neil: if you do not already have the ggplot2 package downloaded on your R, please use the install.packages() function first.
library(ggplot2)
...
```{r, echo = FALSE}
#IGNORE THIS CODE
#NEIL creating list of temp (Only available in DataCamp)
...
Select the option that lists __all__ correct statements.
...
*NOTE FROM NEIL* `vapply()` and `sapply()` are essentially the same. However, `vapply` requires to you fill in a value of your return argument (That is, it wants to make sure you know how many arguments will be retured). Therefore, unlike `sapply`, where it auto defaults to `lapply` when there are multiple arguments returned, it is impossible for `vapply `to return an `lapply` list. Thus, it serves as a safeguard that it did what you wanted it to do. 
...
vapply(temp, basics, numeric(3)) #remember, if it's spitting out numerics, you need to specify that with the "numeric()"!
...
```{r, echo = FALSE}
##Ignore this code
#Neil creating the awards vector (Only available in DataCamp)
...
#Neil creating date vectors (Only available in DataCamp)

day1 <- as.Date(c("2017-09-05"), format = "%Y-%m-%d")
day2 <- as.Date(c("2017-09-07"), format = "%Y-%m-%d")
day3 <- as.Date(c("2017-09-12"), format = "%Y-%m-%d")
day4 <- as.Date(c("2017-09-18"), format = "%Y-%m-%d")
day5 <- as.Date(c("2017-09-23"), format = "%Y-%m-%d")
...
#Neil creating Time vectors (Only available in DataCamp)

login <- as.POSIXct(c(
  "2017-09-10 10:18:04 UTC", "2017-09-15 09:14:18 UTC",
"2017-09-15 12:21:51 UTC", "2017-09-15 12:37:24 UTC",
"2017-09-17 21:37:55 UTC"), 
format = "%Y-%m-%d %T")

logout <- as.POSIXct(c(
"2017-09-10 10:56:29 UTC", "2017-09-15 09:14:52 UTC",
"2017-09-15 12:35:48 UTC", "2017-09-15 13:17:22 UTC",
"2017-09-17 22:08:47 UTC"), 
format = "%Y-%m-%d %T")
...
#IGNORE THIS CODE
#Neil creating meto and astro varibales (Only available in datacamp)

seasons <- c("spring","summer", "fall", "winter")

meteo <- c("March 1, 15",      "June 1, 15" ,"September 1, 15" ,
           "December 1, 15")
